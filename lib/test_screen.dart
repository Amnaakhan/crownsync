...
class EmailMessagesPage extends StatefulWidget {
  @override
  _EmailMessagesPageState createState() => _EmailMessagesPageState();
}
...
class _EmailMessagesPageState extends State<EmailMessagesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: emailMessages.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
        itemCount: emailMessages.length,
        itemBuilder: (context, index) {
          final email = emailMessages[index];
          final logUser = email['log_user'];
          final detail = json.decode(email['detail']);

          return ListTile(
            title: Text(logUser['user_id']['name']),
            subtitle: Text(detail['snippet']),
            onTap: () {
              // Add functionality to open email details page
            },
          );
        },
      ),
